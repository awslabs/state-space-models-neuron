...
def _conv_tile_tensor_e(data_tile, weight_tile, b_tile, dtype, activation=None):
    """Uses the tensor engine to compute and return the convolution of a data tile given the weights.
...
def _conv_tile_scalar_e(data_tile, weight_tile, b_tile, dtype, activation=None):
    """Uses the scalar engine to compute and return the convolution of a data tile given the weights.
...
# select whether to run the convolution on the scalar or tensor engine
...
def conv1d_grouped_kernel(input_data, w, b, output, activation=None):
    ...
    Args:
      ...
      activation: name of the activation function to apply. One of [None, "relu", "silu"].
    """
...
def conv1d_grouped_kernel_longseq(input_data, w, b, output, activation=None):
    """
    NKI kernel to compute grouped 1d causal convolution for long sequences by tiling them. Equivalent to:
...
def conv1d_grouped_kernel_grad(input_data, w, d_output, d_input, d_w, d_b, activation=None):
    ...
    for chunk_id in nl.affine_range(input_data.shape[1] // 128):
        ...
        for batch_id in nl.affine_range(input_data.shape[0]):
            ...
            # Compute d_input
            # todo: avoid the copy and simply reindex the tile to flip it
...
class GroupedConv1dNKI(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias, activation=None):
        # todo: if the output is too large we can avoid storing it and instead recompute it during backprop
        output = torch.empty_like(input)
        conv1d_grouped_kernel_longseq(input, weight, bias, output, activation=activation)
...
class ConvNKI(nn.Conv1d):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, stride: int = 1,
        ...
        """
        Custom layer implemented in NKI to compute efficiently a causal grouped convolution.
        Requires `in_channels == out_channels == groups` and `padding==kernel_size - 1`.
        The output of ConvNKI is equivalent to:
